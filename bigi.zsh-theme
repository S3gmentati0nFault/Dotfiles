...
BLUE_START="${PR_BRIGHT_BLUE}<${PR_BRIGHT_MAGENTA}<${PR_BRIGHT_RED}<${PR_RESET} "
BLUE_START_REVERSE="${PR_BRIGHT_BLUE}>${PR_BRIGHT_MAGENTA}>${PR_BRIGHT_RED}>${PR_RESET}"
BLUE_START_REVERSE_REFLECTED="${PR_BRIGHT_RED}>${PR_BRIGHT_MAGENTA}>${PR_BRIGHT_BLUE}>${PR_RESET}"
GREEN_END="${PR_BLUE}>${PR_GREEN}>${PR_GREY}>${PR_RESET} "

########## VCS ###########
VCS_DIRTY_COLOR="${PR_WHITE}"
VCS_CLEAN_COLOR="${PR_GREEN}"
VCS_COLOR="${PR_BLUE}"
VCS_SUFFIX_COLOR="${VCS_COLOR}› ${PR_RESET}"
...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    echo -n " ${PR_RESET}${PR_RED}%n @ ${PR_BLUE}%m "
  fi
}
...
PROMPT="${BLUE_START}${PR_YELLOW}\$(prompt_header)${PR_RESET}
${BLUE_START_REVERSE_REFLECTED}\$(prompt_context)${PR_BRIGHT_YELLOW}%~${PR_RESET}
${BLUE_START_REVERSE} "
